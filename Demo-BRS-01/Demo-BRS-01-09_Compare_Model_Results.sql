...
SELECT * FROM BRS_2021_prediction_SVM WHERE Game_Result != ML_Prediction ORDER BY Runs_Scored DESC;
...
SELECT * FROM BRS_2021_prediction_RF WHERE Key IN (11,24,25,28,31,42,52,66,67) ORDER BY Runs_Scored DESC;
...
-----+------------+-------------+---------------+---------+-------------+------+----------------+-------+------------+-----------------+--------------------+---------------------+----------------------
  28 | 2021-05-01 |           0 | 0             |      38 |           6 |   12 |              6 |     4 |         12 |           0.316 |              0.372 |               0.447 |                0.819
...
----------------------------------------------------------------------------------------------------------------------------------------------
-- In the above results, both models incorrectly predicted Keys 24 and 25.

-- On Key 24, the Team only scored 2 runs, so it's not surprising that both models incorrectly predicted Loss.
-- On Key 25, the Team only scored 1 run, so it's not surprising that both models incorrectly predicted Loss.

-- (REMEMBER: RF is looking at all the non-response columns while SVM is only looking at Runs_Scored)

-- On Key 28, the Team scored 6 runs, so it's not surprising SVM incorrectly predicted Win, but RF correctly predicted Loss.
-- On Keys 31 and 42, the Team scored 5 runs, so it's not surprising SVM incorrectly predicted Win, but RF correctly predicted Loss.
...
-- On Key 67, the Team only scored 2 runs, so it's not surprising SVM incorrectly predicted Loss, but RF correctly predicted Win.
