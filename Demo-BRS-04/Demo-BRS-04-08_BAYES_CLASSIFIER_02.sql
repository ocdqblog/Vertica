...
DROP TABLE IF EXISTS BRS_2016_2021_Batting_probability_BAYES_metrics;

CREATE TABLE BRS_2016_2021_Batting_probability_BAYES_metrics
AS (SELECT 
    ...
    FROM
    (SELECT COUNT(*) AS TP FROM BRS_2016_2021_Batting_probability_BAYES WHERE Game_Result = ML_Prediction AND Game_Result = 1) AS true_positives,
    (SELECT COUNT(*) AS TN FROM BRS_2016_2021_Batting_probability_BAYES WHERE Game_Result = ML_Prediction AND Game_Result = 0) AS true_negatives,
    (SELECT COUNT(*) AS FP FROM BRS_2016_2021_Batting_probability_BAYES WHERE Game_Result != ML_Prediction AND Game_Result = 0) AS false_positives,
    (SELECT COUNT(*) AS FN FROM BRS_2016_2021_Batting_probability_BAYES WHERE Game_Result != ML_Prediction AND Game_Result = 1) AS false_negatives,
    (SELECT COUNT(*) AS ML_Prediction_Errors FROM BRS_2016_2021_Batting_probability_BAYES WHERE Game_Result != ML_Prediction) AS total_errors);
