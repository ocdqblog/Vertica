...
SELECT CONFUSION_MATRIX(obs::int, pred::int USING PARAMETERS num_classes=2) OVER() 
       FROM (SELECT Game_Result AS obs, PREDICT_XGB_CLASSIFIER (At_Bats,Runs_Scored,Hits,Doubles,Triples,Home_Runs,Runs_Batted_In,Walks,Strikeouts,Hit_By_Pitch,Sacrifice_Hits,Sacrifice_Flies,Reached_On_Error,Grounded_into_Double_Play,Stolen_Bases,Caught_Stealing,Runners_Left_On_Base
...
--------------+-------------+-------------+-----------------------------------------------
            0 |         305 |          35 |
            1 |          44 |         425 | Of 809 rows, 809 were used and 0 were ignored

(2 rows)

DROP TABLE IF EXISTS BRS_2016_2021_Batting_prediction_XGB;

CREATE TABLE BRS_2016_2021_Batting_prediction_XGB AS (SELECT Key, Game_Date, Team, Opponent, Game_Result,
                                                      PREDICT_XGB_CLASSIFIER (At_Bats,Runs_Scored,Hits,Doubles,Triples,Home_Runs,Runs_Batted_In,Walks,Strikeouts,Hit_By_Pitch,Sacrifice_Hits,Sacrifice_Flies,Reached_On_Error,Grounded_into_Double_Play,Stolen_Bases,Caught_Stealing,Runners_Left_On_Base USING PARAMETERS model_name='BRS_2016_2021_Batting_XGBModel') AS ML_Prediction, At_Bats,Runs_Scored,Hits,Doubles,Triples,Home_Runs,Runs_Batted_In,Walks,Strikeouts,Hit_By_Pitch,Sacrifice_Hits,Sacrifice_Flies,Reached_On_Error,Grounded_into_Double_Play,Stolen_Bases,Caught_Stealing,Runners_Left_On_Base   
...
CREATE TABLE BRS_2016_2021_Batting_prediction_XGB_metrics AS (SELECT 
...
----------------------------+-----------------------------+------------------------------+-------------------------------+----------------------+-------------+--------------+-----------------------+----------------
                        425 |                         305 |                           35 |                            44 |                   79 |       0.902 |        0.924 |                 0.906 |          0.897
